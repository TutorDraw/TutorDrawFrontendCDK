// GitHub
export const GITHUB_CONNECTION_ARN =
    "arn:aws:codeconnections:ap-northeast-1:747030889765:connection/6d2df9ca-9e39-4136-86e1-2348f717cfdb";
export const GITHUB_REPO = "TutorDrawCDK";
export const GITHUB_OWNER = "TutorDraw";
export const GITHUB_PACKAGE_BRANCH = "main";
export const GITHUB_FRONTEND_REPO = "TutorDraw-Frontend";

export const APPLICATION_NAME = "TutorDraw";
export const DOMAIN_NAME = "tutordraw.io";

export const STATIC_ASSETS_BUCKET_DEV = "tutordraw-dev-static-website";
export const STATIC_ASSETS_BUCKET_PROD = "tutordraw-prod-static-website";
export const AWS_ACCOUNT = "747030889765";
